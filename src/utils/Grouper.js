jQuery.sap.declare("sap.ui.demo.myFiori.utils.Grouper");
sap.ui.demo.myFiori.utils.Grouper = {
		bundle : null, // somebody has to set this
		LifecycleStatus : function (oContext) { 
			var status = oContext.getProperty("LifecycleStatus"); 
			var text = sap.ui.demo.myFiori.utils.Grouper.bundle.getText("StatusText" + status, "?"); 
		...
		GrossAmount : function (oContext) { 
			...
			if (price <= 5000) { 
				key = "LE10"; text = "< 5000 " + currency; 
			} else if (price > 5000 && price <= 10000) { 
				key = "LE100"; text = "< 10.000 " + currency; 
			} else if (price > 10000) { 
				key = "GT100"; text = "> 10.000 " + currency; 
			} return { 
				key: key, text: text 
			}; 
		} 
